...
class Signature : public Serializable
{
  public:
	...
	explicit Signature(const T& sig)
	    : m_kind(T::Kind), m_signature(sig)
	{
	}
	Signature(const Signature& other)
	    : m_kind(other.m_kind), m_signature(other.m_kind, other.m_signature)
	{
	}
	Signature& operator=(const Signature& o)
	{
		...
		case SignatureKind::Ed25519:
			m_signature.ed25519 = o.m_signature.ed25519;
			break;
		case SignatureKind::Ring:
			m_signature.ring = o.m_signature.ring;
			break;
		}
		return *this;
	}
  ...
  private:
	...
	union Variant
	{
		...
		Variant(SignatureKind k, const Variant& v)
		{
			...
			{
			case SignatureKind::Ed25519:
				new(&ed25519) Ed25519Signature(v.ed25519);
				break;
			case SignatureKind::Ring:
				new(&ring) RingSignature(v.ring);
				break;
			}
		}
	...
	bool Equals(const Signature& o) const
	{
		if( m_kind == o.m_kind )
		{
			...
			{
			case SignatureKind::Ed25519:
				return m_signature.ed25519 == o.m_signature.ed25519;
			case SignatureKind::Ring:
				return m_signature.ring == o.m_signature.ring;
			}
		}
		return false;
	}
  ...
  public:
	~Signature()
	{
		...
		{
		case SignatureKind::Ed25519:
			m_signature.ed25519.~Ed25519Signature();
			break;
		case SignatureKind::Ring:
			m_signature.ring.~RingSignature();
			break;
		}
	}
	...
	void SerializeData(BinaryWriter& writer) const
	{
		...
		{
		case SignatureKind::Ed25519:
			m_signature.ed25519.SerializeData(writer);
			break;
		case SignatureKind::Ring:
			m_signature.ring.SerializeData(writer);
			break;
		}
	}
