...
class Address : public Serializable
{
  public:
	...
	const String& Text() const
	{
		if( _text.empty() )
		{
			...
			case AddressKind::User:
				prefix = PHANTASMA_LITERAL("P");
				break;
			case AddressKind::Interop:
				prefix = PHANTASMA_LITERAL("X");
				break;
			default:
				prefix = PHANTASMA_LITERAL("S");
				break;
			}
			_text.append(prefix);
	...
	{
		PHANTASMA_COPY(o._bytes, o._bytes + LengthInBytes, _bytes);
		_text = o._text;
		return *this;
	}
	...
	static Address FromKey(const Byte* publicKeyBytes, int publicKeyLength)
	{
		...
		else if( publicKeyLength == 34 && publicKeyBytes[0] <= (int)AddressKind::Interop )
		{
			PHANTASMA_COPY(publicKeyBytes, publicKeyBytes + publicKeyLength, bytes);
		}
		else
		{
			PHANTASMA_EXCEPTION("Invalid public key length");
	...
	static Address ForTokenContract(const Char* symbol)
	{
		return FromHash(String{ symbol });
	}
	...
	bool IsSystem() const
	{
		return Kind() == AddressKind::System;
		;
	}
