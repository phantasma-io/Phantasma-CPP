...
class SecureByteArray
{
  public:
	...
	SecureByteArray(int size, const Byte* data = 0, bool protectAccess = true)
	    : m_size(size), m_protectAccess(protectAccess)
	{
		if( size < 0 )
		{
	...
	SecureByteArray(SecureByteArray&& other)
	    : m_data(other.m_data), m_size(other.m_size), m_readers(other.m_readers), m_writers(other.m_writers), m_encrypted(other.m_encrypted), m_protectAccess(other.m_protectAccess)
	{
		other.m_data = 0;
	}
...
class SecureByteReader
{
  public:
	SecureByteReader(SecureByteReader&& other)
	    : data(other.data), owner(other.owner), size(other.size)
	{
		other.data = 0;
		other.owner = 0;
	}
