...
#define CARBON_UNREACHABLE_IF(x)     \
	do                               \
	{                                \
		if( x )                      \
		{                            \
...
#define CARBON_REL_UNREACHABLE \
	do                         \
	{                          \
	} while( 0 )
#define CARBON_REL_UNREACHABLE_IF(x) \
	do                               \
	{                                \
	} while( 0 )
#else
...
#define CarbonAssert(c, ...)                                                                                               \
	do                                                                                                                     \
	{                                                                                                                      \
		bool c_ = (c);                                                                                                     \
		if( !c_ )                                                                                                          \
		{                                                                                                                  \
			::phantasma::carbon::OS_Assert(c_, CARBON_JOIN_DEBUG_INFO("\"" #c "\"", __FILE__, __LINE__) "\n" __VA_ARGS__); \
			CARBON_REL_UNREACHABLE;                                                                                        \
		}                                                                                                                  \
	} while( 0 )
...
struct SmallString {
	...
	SmallString() = default;
	template<int N>
	constexpr SmallString(const char (&sz)[N]) : length(N - 1)
	{
		memcpy(bytes, sz, N - 1);
		static_assert(N <= 256);
	}
...
template<uint32_t N, typename Byte = uint8_t>
struct BytesUpToN {
...
template<int N, typename B = uint8_t>
...
template<int N>
bool operator<(const BytesN<N>& a, const BytesN<N>& b) // lexicographic order
{
	for( size_t i = 0; i != N; ++i )
	{
...
template<int N>
bool operator>=(const BytesN<N>& a, const BytesN<N>& b)
{
	return !(a < b);
}
...
template<int N>
ByteView View(const BytesN<N>& b) { return ByteView{ b.bytes, N }; } // fixed-size byte arrays
template<uint32_t N>
ByteView View(const BytesUpToN<N>& b) { return ByteView{ b.bytes, b.length }; } // variable-size byte arrays
template<class Type, int N>
ByteView View(Type (&b)[N]) { return ByteView{ (uint8_t*)b, sizeof(Type) * N }; } // compile-time fixed arrays
