...
struct StdAllocator {
	...
	template<class Y>
	StdAllocator(const StdAllocator<Y>&) {}

	template<class Y>
	bool operator!=(const StdAllocator<Y>&) const { return false; }
	template<class Y>
	bool operator==(const StdAllocator<Y>&) const { return true; }
...
inline void CryptoRandomBuffer(Bytes& b)
{
	if( !b.empty() )
		CryptoRandomBuffer(&b.front(), b.size());
}
template<class Random>
void RandomBuffer(Bytes& b, Random& r)
{
	if( !b.empty() )
		r.Buffer(&b.front(), b.size());
}

inline ByteView View(const Bytes& b) { return b.empty() ? ByteView{} : ByteView{ &b.front(), b.size() }; }
inline Bytes Clone(ByteView data)
{
	Bytes b;
	if( data.length )
	{
		b.resize(data.length);
		memcpy(&b.front(), data.bytes, data.length);
	}
	return b;
}
inline void Append(Bytes& b, ByteView data)
{
	if( data.length )
	{
		auto s = b.size();
		b.resize(s + data.length);
		memcpy(s + &b.front(), data.bytes, data.length);
	}
}
template<class T>
...
class Allocator
{
  ...
  public:
	...
	void Swap(Allocator& o)
	{
		m_head.swap(o.m_head);
		CARBON_MEMORY_DBG(std::swap(tag, o.tag));
	}
	void Clear();

	template<class T>
	T* Alloc(ptrdiff_t count = 1)
	{
  ...
  private:
	...
	struct Chunk {
		...
		Chunk(Chunk&& c)
		    : a(std::move(c.a)), size(c.size), used(c.used)
		...
		Chunk(std::unique_ptr<uint8_t[]>&& data, size_t size, std::unique_ptr<Chunk>&& p CARBON_MEMORY_DBGC(const char* tag))
		    : a(std::move(data)), size(size), used(0), prev(std::move(p))
	...
	template<class T>
	static T* Alloc(Chunk& chunk, size_t count = 1)
	{
		CarbonAssert(count > 0);
#if !CARBON_MEMORY_BULK_ALLOCATION
...
class ReadView : public ByteView
{
  ...
  public:
	...
	ReadView(void* v, size_t s) : flags(InPlace)
	{
		bytes = (uint8_t*)v;
		length = s;
	}
	ReadView(void* v, size_t s, Allocator& a, Flags f) : allocator(&a), flags(f)
	{
		bytes = (uint8_t*)v;
		length = s;
	}
	...
	explicit ReadView(const ReadView& o) : allocator(o.allocator), fail(o.fail), flags(o.flags)
	{
		length = o.length;
		bytes = o.bytes;
	}
	...
	{
		CarbonAssert(allocator);
		return allocator->Alloc<T>(count);
	}
	ByteView Clone(const ByteView& input)
	{
		CarbonAssert(allocator);
		return allocator->Clone(input);
	}
};

template<class T>
ReadView ReadViewInPlace(const T& b, Allocator& a) { return { View(b), a, ReadView::InPlace }; }
template<class T>
ReadView ReadViewCopy(const T& b, Allocator& a) { return { View(b), a, ReadView::Copy }; }
...
template<class T>
void WriteArray(const vector<T>& vec, WriteView& writer)
{
...
template<class ReadViewT>
bool ReadArray(Bytes& vec, ReadViewT& reader)
{
	uint32_t length = 0;
