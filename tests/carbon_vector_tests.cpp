...
void EncodeTests(TestContext& ctx, const std::vector<Row>& rows)
{
	...
	for( const auto& row : rows )
	{
		...
		bool encoded = true;
		if( row.kind == "U8" )
			Write1((uint8_t)ParseNum(row.value), w);
		else if( row.kind == "I16" )
			Write2((int16_t)ParseNum(row.value), w);
		else if( row.kind == "I32" )
			Write4((int32_t)ParseNum(row.value), w);
		else if( row.kind == "U32" )
			Write4u((uint32_t)ParseUNum(row.value), w);
		else if( row.kind == "I64" )
			Write8((int64_t)ParseNum(row.value), w);
		else if( row.kind == "U64" )
			Write8u((uint64_t)ParseUNum(row.value), w);
		else if( row.kind == "FIX16" )
		{
			auto b = HexToBytes(row.value);
			Write16(b.data(), b.size(), w);
		}
		else if( row.kind == "FIX32" )
		{
			auto b = HexToBytes(row.value);
			Write32(b.data(), b.size(), w);
		}
		else if( row.kind == "FIX64" )
		{
			auto b = HexToBytes(row.value);
			Write64(b.data(), b.size(), w);
		}
		else if( row.kind == "SZ" )
			WriteSz(row.value, w);
		else if( row.kind == "ARRSZ" )
			WriteArraySz(SplitCsv(row.value), w);
		else if( row.kind == "ARR8" )
		{
			std::vector<int8_t> vals;
			for( const auto& s : SplitCsv(row.value) )
				vals.push_back((int8_t)ParseNum(s));
			WriteArray8(vals, w);
		}
		else if( row.kind == "ARR16" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				vals.push_back((int16_t)ParseNum(s));
			WriteArray16(vals, w);
		}
		else if( row.kind == "ARR32" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				vals.push_back((int32_t)ParseNum(s));
			WriteArray32(vals, w);
		}
		else if( row.kind == "ARR64" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				vals.push_back((int64_t)ParseNum(s));
			WriteArray64(vals, w);
		}
		else if( row.kind == "ARRU64" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				vals.push_back(ParseUNum(s));
			WriteArray64u(vals, w);
		}
		else if( row.kind == "ARRBYTES-1D" )
		{
			WriteArray(HexToBytes(row.value), w);
		}
...
void DecodeTests(TestContext& ctx, const std::vector<Row>& rows)
{
	for( const auto& row : rows )
	{
		...
		else if( row.kind == "ARR8" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				expected.push_back((int8_t)ParseNum(s));
		...
		else if( row.kind == "ARR16" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				expected.push_back((int16_t)ParseNum(s));
		...
		else if( row.kind == "ARR32" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				expected.push_back((int32_t)ParseNum(s));
		...
		else if( row.kind == "ARR64" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				expected.push_back((int64_t)ParseNum(s));
		...
		else if( row.kind == "ARRU64" )
		{
			...
			for( const auto& s : SplitCsv(row.value) )
				expected.push_back(ParseUNum(s));
		...
		else if( row.kind == "ARRBYTES-2D" )
		{
			...
			for( const auto& a : arrays )
				got.push_back(BytesToHex(a));
			std::vector<std::string> expected;
			for( const auto& a : ParseArrBytes2D(row.value) )
				expected.push_back(BytesToHex(a));
		...
		else if( row.kind == "VMSTRUCT01" )
		{
			...
			const bool fieldsOk = decoded && SchemaMatches(schemas.seriesMetadata, expected.view.seriesMetadata) && SchemaMatches(schemas.rom, expected.view.rom) && SchemaMatches(schemas.ram, expected.view.ram);
		...
		else if( row.kind == "VMSTRUCT02" )
		{
			...
			const bool fieldsOk = decoded && meta.numFields == 4 && ExpectStringField(meta, "description", "My test token description") && ExpectStringField(meta, "icon", SampleIcon()) && ExpectStringField(meta, "name", "My test token!") && ExpectStringField(meta, "url", "http://example.com");
		...
		else if( row.kind == "TX-CREATE-TOKEN" )
		{
			...
			if( fieldsOk )
			{
				...
				if( fieldsOk )
				{
					...
					fieldsOk = Read(meta, metaReader, tokenAlloc) && meta.numFields == 4 && ExpectStringField(meta, "name", "My test token!") && ExpectStringField(meta, "icon", SampleIcon()) && ExpectStringField(meta, "url", "http://example.com") && ExpectStringField(meta, "description", "My test token description");
				}
				...
				if( fieldsOk )
				{
					...
					const TokenSchemasOwned expected = TokenSchemasBuilder::PrepareStandardTokenSchemas();
					fieldsOk = ReadTokenSchemas(schemas, schemaReader, tokenAlloc) && SchemaMatches(schemas.seriesMetadata, expected.view.seriesMetadata) && SchemaMatches(schemas.rom, expected.view.rom) && SchemaMatches(schemas.ram, expected.view.ram);
				}
		...
		else if( row.kind == "TX-CREATE-TOKEN-SERIES" )
		{
			...
			if( fieldsOk )
			{
				...
				if( fieldsOk )
				{
					fieldsOk = ReadSeriesInfo(seriesInfo, argsReader, seriesAlloc) && seriesInfo.maxMint == 0 && seriesInfo.maxSupply == 0 && seriesInfo.owner == senderPub && seriesInfo.rom.numFields == 0 && seriesInfo.ram.numFields == 0;
				}
				...
				if( fieldsOk )
				{
					...
					VmDynamicStruct meta{};
					fieldsOk = Read(meta, schemas.view.seriesMetadata, metaReader, seriesAlloc) && meta.numFields == 3 && ExpectInt256Field(meta, StandardMeta::id.c_str(), ToInt256(BigInteger::Parse(String("115792089237316195423570985008687907853269984665640564039457584007913129639935")))) && ExpectInt8Field(meta, "mode", 0) && ExpectBytesField(meta, "rom", ByteArray{});
				}
		...
		else if( row.kind == "TX-MINT-NON-FUNGIBLE" )
		{
			...
			if( fieldsOk )
			{
				fieldsOk = msg.mintNonFungible.tokenId == (uint64_t)-1 && msg.mintNonFungible.seriesId == 0xFFFFFFFFu && msg.mintNonFungible.to == senderPub && msg.mintNonFungible.ram.length == 0;
			}
			...
			if( fieldsOk )
			{
				...
				VmDynamicStruct rom{};
				fieldsOk = Read(rom, schemas.view.rom, romReader, alloc) && ExpectInt256Field(rom, StandardMeta::id.c_str(), ToInt256(BigInteger::Parse(String("115792089237316195423570985008687907853269984665640564039457584007913129639935")))) && ExpectBytesField(rom, "rom", ByteArray{ (Byte)0x01, (Byte)0x42 }) && ExpectStringField(rom, "name", "My NFT #1") && ExpectStringField(rom, "description", "This is my first NFT!") && ExpectStringField(rom, "imageURL", "images-assets.nasa.gov/image/PIA13227/PIA13227~orig.jpg") && ExpectStringField(rom, "infoURL", "https://images.nasa.gov/details/PIA13227") && ExpectInt32Field(rom, "royalties", 10000000);
			}
